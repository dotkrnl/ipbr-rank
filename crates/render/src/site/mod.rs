...
pub(crate) fn layout(title: &str, scoreboard: &Scoreboard, body: &str) -> String {
    format!(
        r#"<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title}</title><link rel="stylesheet" href="assets/style.css"><script defer src="assets/app.js"></script></head><body data-mode="raw"><div class="shell"><header><div class="brand"><a class="brand-link" href="index.html"><span class="prompt">$</span>ipbr-rank · live llm coding-role score</a><div class="meta">refreshed <time datetime="{generated_at}" data-local-time>{generated_at}</time> · {source_count} sources · updated frequently — models drift and degrade</div></div><nav><a href="about.html">about</a><a href="scoreboard.toml">api</a><a href="https://github.com/dotkrnl/ipbr-rank" rel="noopener noreferrer">github</a></nav></header><main>{body}</main></div></body></html>"#,
